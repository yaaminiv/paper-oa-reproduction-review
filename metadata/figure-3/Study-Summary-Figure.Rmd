...
## Count the number of fertilization studies

```{r}
fertilizationMetadata <- allMetadata %>%
  filter(., category == "Fertilization") %>%
  select(., !c(effect, taxa)) %>%
  distinct(.) #Get fertilization data, remove effect and taxa information, keep distinct rows
nrow(fertilizationMetadata) #92 fertilization studies
```
...
```{r}
barplotColors <- c("grey90", "grey30") #Create a BW palette with 2 colors to represent effect and no effect of OA on reproductive processes
barplot(t(t(findingSummary[9:10, 3:6])), ylim = c(0,5),
...
axis(side = 2, at = c(0,1,3,5), labels = c(0,1,3,5), las = 2, col = "grey80", cex.axis = 2.5) #Add y-axis
...
legend(x = 0.05, y = 4, 
       legend = c("No Effect", "Effect"), cex = 2.3,
       pch = 22, pt.cex = 4.5,
        col = "black",
...
#Gametogenesis
barplot(t(t(findingSummary[5:6, 3:6])), ylim = c(0,25),
        axes = FALSE, axisnames = FALSE,
        col = barplotColors) #Create a stacked barplot using colors in barplotColors. Do not plot any axes and increase font size of x-axis labels
axis(side = 2, at = c(0,5,15,25), labels = c(0,5,15,25), las = 2, col = "grey80", cex.axis = 2.5) #Add y-axis
...
barplot(t(t(findingSummary[1:2, 3:6])),  
        axes = FALSE, axisnames = FALSE,
        col = barplotColors) #Create a stacked barplot using colors in barplotColors. Do not plot any axes and increase font size of x-axis labels
axis(side = 2, at = c(0,10,20,40), labels = c(0,10,20,40), las = 2, col = "grey80", cex.axis = 2.5) #Add y-axis
mtext(side = 3, " Fecundity", adj = 0, line = -1, cex = 1.5) #Add category name
#mtext(side = 3, expression(italic("  ? studies")), adj = 0, line = -3, cex = 1.2) #Add total studies

#Timing
barplot(t(t(findingSummary[11:12, 3:6])), ylim = c(0,5), 
        axes = FALSE, axisnames = FALSE,
        col = barplotColors) #Create a stacked barplot using colors in barplotColors. Do not plot any axes and increase font size of x-axis labels
axis(side = 2, at = c(0,1,3,5), labels = c(0,1,3,5), las = 2, col = "grey80", cex.axis = 2.5) #Add y-axis
...
#Mating behavior
barplot(t(t(findingSummary[7:8, 3:6])), ylim = c(0,5),   
        axes = FALSE, 
...
#Fertilization
barplot(t(t(findingSummary[3:4, 3:6])), ylim = c(0,70),
        axes = FALSE, 
        names.arg = c("N", "C", "E", "M"), cex.names = 2.5,
        col = barplotColors) #Create a stacked barplot using colors in barplotColors. Do not plot any axes and increase font size of x-axis labels
axis(side = 2, at = c(0,10,40,70), labels = c(0,10,40,70), las = 2, col = "grey80", cex.axis = 2.5) #Add y-axis
...
yearMetadata <- allMetadata %>% 
  ...
  distinct(., taxa, DOI, .keep_all = TRUE) %>%
  select(., taxa, year) #Select taxa, year and DOI columns, keep rows with unique DOI (remove duplicates generated by reproduction category findings but keep duplicate DOI if it's for different taxa), and remove DOI column
head(yearMetadata) #Confirm formatting
```

```{r}
#Group yearMetadata by taxa and year, count studies per taxa/year, make taxa columns, replace NA with 0, reorganize by year, remove 1924, add blank rows for 2005/2006, reorganize by year, then add cumulative sums for each taxa and save as another column
...
yearPublished <- yearMetadata %>%
  ...
  arrange(., year) %>%
  slice(., -1) %>%
  add_row(year = 2005, cnidarian = 0, crustacean = 0, echinoderm = 0, mollusc = 0, .after = 1) %>%
  add_row(year = 2006, cnidarian = 0, crustacean = 0, echinoderm = 0, mollusc = 0, .after = 2) %>%
  mutate(., 
...
axis(side = 1, at = 1:18, labels = yearPublished$year, las = 2, col = "grey80", cex.axis = 2) #Add an x-axis with year information
...
```{r}
locationMetadata <- allMetadata %>% 
  select(., taxa, collectionLocation, DOI) %>%
  distinct(., taxa, DOI, .keep_all = TRUE) %>%
  select(., taxa, collectionLocation) #Select taxa, collectionLocation and DOI columns, keep rows with unique taxa + DOI combinations (remove duplicates generated by reproduction category findings but keep duplicate DOI if it's for different taxa), and remove DOI column
head(locationMetadata) #Confirm formatting
```

```{r}
#Group locationMetadata by taxa and collectionLocation, count studies per taxa/year, make taxa columns, replace NA with 0, reorganize by collection location, remove NA collection locations (for Smith and Clowes 1924), add numeric region information, latitude, and longitude information
...
```{r}
locationData <- locationData %>% mutate(totalStudies = rowSums(locationData[,2:5])) #Add total number of studies
tail(locationData) #Confirm formatting
```
...
scatterpieData$radius <- ((rowSums(x = scatterpieData[,4:7]))/2) #Scale pie radius based on number of observations for each region, and divide by 2 so pies are not too large
...
```{r}
pieColors <- RColorBrewer::brewer.pal(4, "RdBu") #Create a red-blue palette with 4 colors to represent each taxa
pieColors
```
