...
locationMetadata <- allMetadata %>% 
  ...
  distinct(., DOI, .keep_all = TRUE) %>%
  select(., taxa, collectionLocation) #Select taxa, collectionLocation and DOI columns, keep rows with unique DOI (remove duplicates generated by reproduction category findings), and remove DOI column
head(locationMetadata) #Confirm formatting
```

```{r}
#Group locationMetadata by taxa and collectionLocation, count studies per taxa/year, make taxa columns, replace NA with 0, reorganize by collection location, remove NA collection locations, add numeric region information, latitude, and longitude information
...
locationData <- locationMetadata %>%
  ...
  mutate(., 
         ...
         totalStudies = rowSums(locationData[,2:5]))
